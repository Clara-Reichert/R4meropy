...
#Lecture des données comptées
C<- read.csv("C:/Users/clare/Desktop/mais/mais_csv/mais_train_counted2.csv", sep=";")
#enlever une ligne qui correspond pas
C=C[-23,]
#faire correspondre les noms des images
M=match(C$name, IMJ$Slice)
C$imagej=IMJ$Count
C$diff=abs(C$tag-C$imagej)
moy=mean(C$diff)  

Files=list.files(path="C:\\Users\\clare\\Desktop\\mais\\mais_img_preprocessed\\train")
Files=Files[-135]
...
write.csv2(M2, file = "mais_coord_train.csv", row.names = FALSE)
